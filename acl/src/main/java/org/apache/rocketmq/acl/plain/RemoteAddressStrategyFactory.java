...
public class RemoteAddressStrategyFactory {
  ...
  public static final BlankRemoteAddressStrategy BLANK_NET_ADDRESS_STRATEGY =
      new BlankRemoteAddressStrategy();
      ...
      if (AclUtils.isColon(remoteAddr)) {
        ...
        if (!last.startsWith("{")) {
          throw new AclException(
              String.format(
                  "MultipleRemoteAddressStrategy netaddress examine scope Exception netaddress",
                  remoteAddr));
        }
        ...
        if (strArray.length < 4) {
          throw new AclException(
              String.format(
                  "MultipleRemoteAddressStrategy has got a/some wrong format IP(s)", remoteAddr));
        }
  ...
  /**
   * 表示全部匹配，该条规则直接 返回 true，将会阻断其他规则的判断，请慎重使用，其配置风格类似 于：
   *
   * <ul>
   *   <li>ALL:"*"
   *   <li>IPV4:"*.*.*.*"
  ...
  /**
   * 多地址配置模式，IP 地址 的最后一组使用 {}，大括号中可以包含多个 IP 地址用英文逗号隔开， 例如 192.168.0.{100,101}
   *
   * @author shui4
   */
    ...
    public MultipleRemoteAddressStrategy(String[] strArray) {
      ...
      for (String netaddress : strArray) {
        if (validator.isValidInet4Address(netaddress)) {
          ...
          throw new AclException(
              String.format("Netaddress examine Exception netaddress is %s", netaddress));
        }
      }
    }
  ...
  public static class OneRemoteAddressStrategy implements RemoteAddressStrategy {
    ...
    @Override
    public boolean match(PlainAccessResource plainAccessResource) {
      String writeRemoteAddress =
          AclUtils.expandIP(plainAccessResource.getWhiteRemoteAddress(), 8).toUpperCase();
      return AclUtils.expandIP(netaddress, 8).toUpperCase().equals(writeRemoteAddress);
    }
  }

  /**
   * 范围类地址配置模式，例如 192.168.*. 或 192.168.100-200.10-20
   *
   * @author shui4
   */
    ...
    private boolean analysis(String[] strArray, int index) {
      ...
      if ("*".equals(value)) {
        ...
        if (value.indexOf("-") == 0) {
          throw new AclException(
              String.format(
                  "RangeRemoteAddressStrategy netaddress examine scope Exception value %s", value));
        }
