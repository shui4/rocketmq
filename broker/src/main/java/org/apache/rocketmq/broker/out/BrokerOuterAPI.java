...
public class BrokerOuterAPI {
  ...
  public List<RegisterBrokerResult> registerBrokerAll(
    ...
    if (nameServerAddressList != null && nameServerAddressList.size() > 0) {
      ...
      requestHeader.setBrokerId(brokerId); // brokerId=0表示主节点，brokerId>0表示从节点
      ...
      //      topicConfigWrapper，主题配置，topicConfigWrapper内部封装
      //      的是TopicConfigManager中的topicConfigTable，内部存储的是
      //      Broker启动时默认的一些topic Broker中topic默认存储在 ${Rocket_Home}/store/config/topics.json中
                ...
                try {
                  ...
                  RemotingCommand request =
                      RemotingCommand.createRequestCommand(
                          RequestCode.QUERY_DATA_VERSION, requestHeader);
                  ...
                  log.warn(
                      "Query data version from name server {} OK,changed {}, broker {},name server {}",
                      namesrvAddr,
                      changed,
                  ...
                  changedList.add(Boolean.TRUE);
                  log.error(
                      "Query data version from name server {}  Exception, {}", namesrvAddr, e);
                } finally {
                  countDownLatch.countDown();
                }
              }
  ...
  public TopicConfigSerializeWrapper getAllTopicConfig(final String addr)
    ...
    RemotingCommand response =
        this.remotingClient.invokeSync(MixAll.brokerVIPChannel(true, addr), request, 3000);
    assert response != null;
    switch (response.getCode()) {
      case ResponseCode.SUCCESS:
        {
  ...
  public ConsumerOffsetSerializeWrapper getAllConsumerOffset(final String addr)
    ...
    switch (response.getCode()) {
      case ResponseCode.SUCCESS:
        {
          return ConsumerOffsetSerializeWrapper.decode(
              response.getBody(), ConsumerOffsetSerializeWrapper.class);
        }
      default:
        break;
    }
