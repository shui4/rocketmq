...
public class PullMessageProcessor extends AsyncNettyRequestProcessor
  ...
  private RemotingCommand processRequest(
      final Channel channel, RemotingCommand request, boolean brokerAllowSuspend)
      throws RemotingCommandException {
    ...
    if (!PermName.isReadable(this.brokerController.getBrokerConfig().getBrokerPermission())) {
      ...
      response.setRemark(
          String.format(
              "the broker[%s] pulling message is forbidden",
    ...
    final long suspendTimeoutMillisLong =
        hasSuspendFlag ? requestHeader.getSuspendTimeoutMillis() : 0;
    ...
    if (!PermName.isReadable(topicConfig.getPerm())) {
      response.setCode(ResponseCode.NO_PERMISSION);
      response.setRemark(
          "the topic[" + requestHeader.getTopic() + "] pulling message is forbidden");
      return response;
    }
    ...
    if (hasSubscriptionFlag) {
      ...
      if (null == consumerGroupInfo) {
        log.warn(
            "the consumer's group info not exist, group: {}", requestHeader.getConsumerGroup());
      ...
      if (!ExpressionType.isTagType(subscriptionData.getExpressionType())) {
        ...
        if (consumerFilterData.getClientVersion() < requestHeader.getSubVersion()) {
          log.warn(
              ...
              requestHeader.getConsumerGroup(),
              requestHeader.getTopic(),
              consumerFilterData.getClientVersion(),
              requestHeader.getSubVersion());
    ...
    if (this.brokerController.getBrokerConfig().isFilterSupportRetry()) {
      messageFilter =
          new ExpressionForRetryMessageFilter(
              subscriptionData,
              consumerFilterData,
    ...
    if (getMessageResult != null) {
      ...
      // 如果主服务器繁忙则建议下一次从从服务器拉取消息，设置 suggestWhichBrokerId 为配置文件中的
      // whichBrokerWhenConsumeSlowly 属性，默认为 1。如果一个主服务器拥
      // 有多台从服务器，参与消息拉取负载的从服务器只会是其中一个。
  ...
  private byte[] readGetMessageResult(
    ...
    try {
      ...
      for (ByteBuffer bb : messageBufferList) {
        ...
        //                bornhost has the IPv4 ip if the MessageSysFlag.BORNHOST_V6_FLAG bit of
        // sysFlag is 0
        //                IPv4 host = ip(4 byte) + port(4 byte); IPv6 host = ip(16 byte) + port(4
        // byte)
    ...
    try {
      ...
      msgInner.setTagsCode(
          MessageExtBrokerInner.tagsString2tagsCode(
              TopicFilterType.SINGLE_TAG, msgInner.getTags()));

      msgInner.setQueueId(0);
      msgInner.setSysFlag(0);
      msgInner.setBornTimestamp(System.currentTimeMillis());
      msgInner.setBornHost(
          RemotingUtil.string2SocketAddress(this.brokerController.getBrokerAddr()));
