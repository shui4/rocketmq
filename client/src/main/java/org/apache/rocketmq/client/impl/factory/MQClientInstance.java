...
public class MQClientInstance {
  ...
  public MQClientInstance(
      ClientConfig clientConfig, int instanceIndex, String clientId, RPCHook rpcHook) {
    ...
    this.mQClientAPIImpl =
        new MQClientAPIImpl(
            this.nettyClientConfig, this.clientRemotingProcessor, rpcHook, clientConfig);
  ...
  public ConsumeMessageDirectlyResult consumeMessageDirectly(
      final MessageExt msg, final String consumerGroup, final String brokerName) {
  ...
  public synchronized boolean registerProducer(
      final String group, final DefaultMQProducerImpl producer) {
  ...
  public synchronized void resetOffset(
      String topic, String group, Map<MessageQueue, Long> offsetTable) {
      ...
      while (iterator.hasNext()) {
        ...
        if (topic.equals(mq.getTopic()) && offset != null) {
          try {
            ...
            consumer
                .getRebalanceImpl()
                .removeUnnecessaryMessageQueue(mq, processQueueTable.get(mq));
            iterator.remove();
          } catch (Exception e) {
        ...
        if (oneTable != null) {
          for (Map.Entry<Long, String> entry1 : oneTable.entrySet()) {
            ...
            if (addr != null) {
              ...
              try {
                int version =
                    this.mQClientAPIImpl.sendHearbeat(
                        addr, heartbeatData, clientConfig.getMqClientApiTimeout());
                if (!this.brokerVersionTable.containsKey(brokerName)) {
      ...
      if (ConsumeType.CONSUME_PASSIVELY == consumer.consumeType()) {
        ...
        for (SubscriptionData sub : subscriptions) {
          if (sub.isClassFilterMode() && sub.getFilterClassSource() != null) {
            ...
            try {
              this.uploadFilterClassToAllFilterServer(
   ...
   * <code>subscribe(final String topic, final MessageSelector messageSelector)</code> is
   * recommended.
   */
  @Deprecated
  private void uploadFilterClassToAllFilterServer(
    ...
    try {
      ...
      log.warn(
          "uploadFilterClassToAllFilterServer Exception, ClassName: {} {}",
          fullClassName,
        ...
        for (final String fsAddr : value) {
          try {
            this.mQClientAPIImpl.registerMessageFilterClass(
                fsAddr, consumerGroup, topic, fullClassName, classCRC, classBody, 5000);
        ...
        for (Map.Entry<Long, String> entry1 : oneTable.entrySet()) {
          ...
          if (addr != null) {
            try {
              ...
              log.warn(
                  "unregister client RemotingException from broker: {}, {}", addr, e.getMessage());
            } catch (InterruptedException e) {
              log.warn(
                  "unregister client InterruptedException from broker: {}, {}",
                  addr,
                  e.getMessage());
            } catch (MQBrokerException e) {
              log.warn(
                  "unregister client MQBrokerException from broker: {}, {}", addr, e.getMessage());
            }
          }
        }
      }
