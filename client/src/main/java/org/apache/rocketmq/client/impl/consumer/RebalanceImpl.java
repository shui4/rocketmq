...
public abstract class RebalanceImpl {
      ...
      case CLUSTERING:
        {
          // 从主题订阅信息缓存表中获取主题的队列信息
          Set<MessageQueue> mqSet = this.topicSubscribeInfoTable.get(topic);
          // 发送请求从 Broker 中获取该消费组内当前所有的消费者客户端 ID
          ...
          if (mqSet != null && cidAll != null) {
            ...
            mqAll.addAll(mqSet);
            // 对 cidAll 、 mqAll 进行排序。这一步很重要，同一个消费组内看到的视图应保持一致，确保同一个消费队列不会被多个消费者分配
  ...
  private boolean updateProcessQueueTableInRebalance(
      final String topic, final Set<MessageQueue> mqSet, final boolean isOrder) {
    boolean changed = false;
    // processQueueTable 是当前消费者负载的消息队列缓存表，
    ...
    while (it.hasNext()) {
      ...
      if (mq.getTopic().equals(topic)) {
        // 如果缓存表中的 MessageQueue 不包含在 mqSet 中，说明经过本次消息队列负载后，该 mq 被分配给其他消费者，需要暂停该消息队列消息的消费
        if (!mqSet.contains(mq)) {
          // 将 ProccessQueue 的状态设置为 droped=true ，该 ProcessQueue 中的消息将不会再被消费，调用
    ...
    for (MessageQueue mq : mqSet) {
      ...
      if (!this.processQueueTable.containsKey(mq)) {
        if (isOrder && !this.lock(mq)) {
          log.warn(
              "doRebalance, {}, add a new mq failed, {}, because lock failed", consumerGroup, mq);
          continue;
        }
        // 首先从内存中移除该消息队列的消费进度，
        ...
        // 这里有一个关键，如果读取到的消费进度小于 0 ，则需要校对消费进度。 RocketMQ 提供了
        // CONSUME_FROM_LAST_OFFSET 、 CONSUME_FROM_FIRST_OFFSET 、
        // CONSUME_FROM_TIMESTAMP 方式，在创建消费者时可以通过调用
        // DefaultMQPushConsumer#setConsumeFromWhere 方法进行设置
        if (nextOffset >= 0) {
  ...
  public abstract void messageQueueChanged(
      final String topic, final Set<MessageQueue> mqAll, final Set<MessageQueue> mqDivided);

  public abstract boolean removeUnnecessaryMessageQueue(
      final MessageQueue mq, final ProcessQueue pq);
   ...
   */
  public abstract long computePullFromWhereWithException(final MessageQueue mq)
      throws MQClientException;

  public abstract void dispatchPullRequest(final List<PullRequest> pullRequestList);

  /**
   * When the network is unstable, using this interface may return wrong offset. It is recommended
   * to use computePullFromWhereWithException instead.
   *
   * @param mq
   * @return offset
   */
