...
public class DefaultMQPushConsumer extends ClientConfig implements MQPushConsumer {
   ...
   * subscribed, which achieves load balances; Conversely, if the broadcasting is set, each consumer
   * client will consume all subscribed messages separately. This field defaults to clustering.
   */
   ...
   *
   * <ul>
   *   <li><code>CONSUME_FROM_LAST_OFFSET</code>: consumer clients pick up where it stopped
   *       previously. If it were a newly booting up consumer client, according aging of the
   ...
   *         <li>if the earliest message being subscribed has expired, consuming will start from the
   *             latest messages, meaning messages born prior to the booting timestamp would be
   *             ignored.
   *       </ol>
   *   <li><code>CONSUME_FROM_FIRST_OFFSET</code>: Consumer client will start from earliest messages
   *       available.
   *   <li><code>CONSUME_FROM_TIMESTAMP</code>: Consumer client will start from specified timestamp,
   *       which means messages born prior to {@link #consumeTimestamp} will be ignored
   * </ul>
   */
  ...
  /**
   * Constructor specifying namespace, consumer group, RPC hook and message queue allocating
   * algorithm.
   *
  ...
  /**
   * Constructor specifying namespace, consumer group, RPC hook, message queue allocating algorithm,
   ...
   */
  public DefaultMQPushConsumer(
      final String consumerGroup, boolean enableMsgTrace, final String customizedTraceTopic) {
    this(
        null,
        consumerGroup,
        null,
  ...
  /**
   * Constructor specifying consumer group, RPC hook, message queue allocating algorithm, enabled
   * msg trace flag and customized trace topic name.
   *
   * @param consumerGroup Consume queue.
   * @param rpcHook RPC hook to execute before each remoting command.
   * @param allocateMessageQueueStrategy message queue allocating algorithm.
  ...
  public void createTopic(String key, String newTopic, int queueNum, int topicSysFlag)
    ...
    this.defaultMQPushConsumerImpl.createTopic(
        key, withNamespace(newTopic), queueNum, topicSysFlag);
  }
  ...
  public QueryResult queryMessage(String topic, String key, int maxNum, long begin, long end)
    ...
    return this.defaultMQPushConsumerImpl.queryMessage(
        withNamespace(topic), key, maxNum, begin, end);
  }
   ...
   *
   * <p>This method will be removed or it's visibility will be changed in a certain version after
   * April 5, 2020, so please do not use this method.
   *
  ...
  /**
   * Send message back to the broker whose name is <code>brokerName</code> and the message will be
   * re-delivered in future.
   *
   * <p>This method will be removed or it's visibility will be changed in a certain version after
   * April 5, 2020, so please do not use this method.
   *
  ...
  /**
   * This method gets internal infrastructure readily to serve. Instances must call this method
   * after configuration.
   *
   * @throws MQClientException if there is any client error.
   */
  ...
  public void subscribe(String topic, String fullClassName, String filterClassSource)
      throws MQClientException {
    this.defaultMQPushConsumerImpl.subscribe(
        withNamespace(topic), fullClassName, filterClassSource);
  }
