...
public class RebalancePushImpl extends RebalanceImpl {
  private static final long UNLOCK_DELAY_TIME_MILLS =
  ...
  public void messageQueueChanged(
    ...
    /**
     * When rebalance result changed, should update subscription's version to notify broker. Fix:
     * inconsistency subscription may lead to consumer miss messages.
     */
    ...
    if (currentQueueCount != 0) {
      ...
      if (pullThresholdSizeForTopic != -1) {
        ...
        this.defaultMQPushConsumerImpl
            .getDefaultMQPushConsumer()
            .setPullThresholdSizeForQueue(newVal);
      }
    }
  ...
  public long computePullFromWhereWithException(MessageQueue mq) throws MQClientException {
    ...
    // 注意：ConsumeFromWhere相关消费进度校正策略只有在从磁盘中获取消费进
    // 度返回-1时才会生效，如果从消息进度存储文件中返回的消费进度小
    // 于-1，表示偏移量非法，则使用偏移量-1去拉取消息，那么会发生什
    // 么呢？首先第一次去消息服务器拉取消息时无法取到消息，但是会
    // 用-1去更新消费进度，然后将消息消费队列丢弃，在下一次消息队列
    // 负载时再次消费。
      ...
      case CONSUME_FROM_TIMESTAMP:
        {
          ...
          // 返回0。如果小于-1，表示该消息进度文件中存储了错误的偏移量，则返回-1
          long lastOffset = offsetStore.readOffset(mq, ReadOffsetType.READ_FROM_STORE);
          if (lastOffset >= 0) {
            result = lastOffset;
          } else if (-1 == lastOffset) {
