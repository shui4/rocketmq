...
/**
 * 一种基于机房近侧优先级的分配策略代理。可以指定实际的分配策略。如果某个机房中有任何消费者存活，那么部署在同一台机器上的broker的消息队列应该只分配给这些消费者。否则，这些消息队列可以由所有消费者共享，因为没有活着的消费者来垄断它们。
 */
public class AllocateMachineRoomNearby implements AllocateMessageQueueStrategy {
  ...
  public AllocateMachineRoomNearby(
  ...
  public List<MessageQueue> allocate(
    ...
    Map<String /*machine room */, List<MessageQueue>> mr2Mq =
        new TreeMap<String, List<MessageQueue>>();
    ...
    Map<String /*machine room */, List<String /*clientId*/>> mr2c =
    ...
    // 2.allocate the rest mq to each machine room if there are no consumer alive in that machine
    // room
    for (Entry<String, List<MessageQueue>> machineRoomEntry : mr2Mq.entrySet()) {
      if (!mr2c.containsKey(
          machineRoomEntry
              .getKey())) { // no alive consumer in the corresponding machine room, so all consumers
        // share these queues
        allocateResults.addAll(
            allocateMessageQueueStrategy.allocate(
                consumerGroup, currentCID, machineRoomEntry.getValue(), cidAll));
      }
    }
  ...
  /**
   * A resolver object to determine which machine room do the message queues or clients are deployed
   * in.
   *
   * <p>AllocateMachineRoomNearby will use the results to group the message queues and clients by
   * machine room.
   *
   * <p>The result returned from the implemented method CANNOT be null.
   */
