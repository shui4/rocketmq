...
/**
 * This class is the entry point for applications intending to send messages. It's fine to tune
 * fields which exposes getter/setter methods, but keep in mind, all of them should work well out of
 * box for most scenarios. This class aggregates various <code>send</code> methods to deliver
 * messages to brokers. Each of them has pros and cons; you'd better understand strengths and
 * weakness of them before actually coding.
 *
 * <p><strong>Thread Safety:</strong> After configuring and starting process, this class can be
 * regarded as thread-safe and used among multiple threads context.
 */
public class DefaultMQProducer extends ClientConfig implements MQProducer {
   ...
   */
  public DefaultMQProducer(
      final String producerGroup, boolean enableMsgTrace, final String customizedTraceTopic) {
  ...
  /**
   * Create a topic on broker. This method will be removed in a certain version after April 5, 2020,
   ...
   *
   * <p>This method will be removed in a certain version after April 5, 2020, so please do not use
   * this method.
   *
   * @param mq Instance of MessageQueue
  ...
  /**
   * Send message to broker asynchronously. This method returns immediately. On sending completion,
   * <code>sendCallback</code> will be executed. Similar to {@link #send(Message)}, internal
   * implementation would potentially retry up to {@link #retryTimesWhenSendAsyncFailed} times
   * before claiming sending failure, which may yield message duplication and application developers
   * are the one to resolve this potential issue.
   *
   * @param msg Message to send.
  ...
  public TransactionSendResult sendMessageInTransaction(
      Message msg, LocalTransactionExecuter tranExecuter, final Object arg)
      throws MQClientException {
  ...
  /**
   * Send request message in synchronous mode. This method returns only when the consumer consume
   * the request message and reply a message. <strong>Warn:</strong> this method has internal
   * retry-mechanism, that is, internal implementation will retry {@link #retryTimesWhenSendFailed}
   * times before claiming failure. As a result, multiple messages may potentially delivered to
   * broker(s). It's up to the application developers to resolve potential duplication issue.
   *
  ...
  /**
   * Request asynchronously. This method returns immediately. On receiving reply message, <code>
   * requestCallback</code> will be executed. Similar to {@link #request(Message, long)}, internal
   * implementation would potentially retry up to {@link #retryTimesWhenSendAsyncFailed} times
   * before claiming sending failure, which may yield message duplication and application developers
   * are the one to resolve this potential issue.
   *
  ...
  /**
   * Same to {@link #request(Message, RequestCallback, long)} with target message selector
   * specified.
   *
