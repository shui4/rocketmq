...
public interface MessageStore {
  ...
  /**
   * Store a message into store in async manner, the processor can process the next request rather
   * than wait for result when result is completed, notify the client in async manner
   *
   ...
   * @return a CompletableFuture for the result of store operation
   */
  default CompletableFuture<PutMessageResult> asyncPutMessages(
  ...
  long getCommitLogOffsetInQueue(
      final String topic, final int queueId, final long consumeQueueOffset);
  ...
  /**
   * Get the raw commit log data starting from the given offset, which should used for replication
   * purpose.
   *
   ...
   * @return true if success; false otherwise.
   */
  boolean appendToCommitLog(
      final long startOffset, final byte[] data, int dataStart, int dataLength);
   ...
   */
  QueryMessageResult queryMessage(
  ...
  /**
   * Get number of the bytes that have been stored in commit log and not yet dispatched to consume
   * queue.
   *
  ...
  /**
   * Clean unused lmq topic. When calling to clean up the lmq topic, the lmq topic cannot be used to
   * write messages at the same time, otherwise the messages of the cleaning lmq topic may be lost,
