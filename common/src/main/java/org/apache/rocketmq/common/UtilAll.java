...
public class UtilAll {
  ...
  static final String HOST_NAME =
      ManagementFactory.getRuntimeMXBean().getName(); // format: "pid@hostname"
  ...
  public static double getDiskPartitionSpaceUsedPercent(final String path) {
    ...
    try {
      ...
      if (!file.exists()) {
        log.error(
            "Error when measuring disk space usage, file doesn't exist on this path: {}", path);
        return -1;
      }
      ...
      if (totalSpace > 0) {
        ...
        long roundNum = 0;
        // 如果有余数，后续占比额外 +1 ，（这里没有不四舍五入，没有使用 BigDecimal ）
    ...
    try {
      ...
      while (ite.hasNext()) {
        ...
        if (elements != null && elements.length > 0) {
          ...
          result.append(
              String.format(
                  "%-40sTID: %d STATE: %s%n", threadName, thread.getId(), thread.getState()));
