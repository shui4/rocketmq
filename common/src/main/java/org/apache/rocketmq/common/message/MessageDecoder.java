...
public class MessageDecoder {
  ...
  public static String createMessageId(
      final ByteBuffer input, final ByteBuffer addr, final long offset) {
    input.flip();
  ...
  /**
   * 代码清单3-31 消息编码结构<br>
   * 在创建 {@link RemotingCommand} 对象时，调用{@link MessageBatch#encode()} 方法填充到 {@link
   * RemotingCommand#setBody(byte[])}中
   */
  public static byte[] encodeMessage(Message message) {
    ...
    if (propsLen > Short.MAX_VALUE)
      throw new RuntimeException(
          String.format(
              "Properties size of message exceeded, properties size: {}, maxSize: {}.",
              propsLen,
              Short.MAX_VALUE));
